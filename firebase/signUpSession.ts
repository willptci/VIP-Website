// File: src/firebase/signUpSession.ts
import { db } from './firebaseConfig';
import { doc, getDoc, setDoc, deleteDoc, serverTimestamp, writeBatch, } from 'firebase/firestore';
import { signUp } from '@/lib/actions/user.actions';
import { sendTextMessage } from '@/app/api/whatsapp/messages/textMessages';
import { number } from 'zod';
// import { sendUrlButton } from '@/app/api/whatsapp/messages/ctaMessages';

export interface sessionType {
    phone: string,
    step: 'init',
    data: {},
    createdAt: typeof serverTimestamp,
    expiresAt: typeof serverTimestamp,   // human‐readable
    ttl: number, // Firestore TTL
}

const SESSIONS = 'sessions';
const BUSINESSES = 'businesses';
const SETTINGS = 'settings';
// const PLATFORM = 'https://9111-2601-c2-1680-6460-95ce-b2cb-de73-bda3.ngrok-free.app/';

// Retrieve a session for a user by phone number
export async function getSessionForUser(phone: string) {
  const ref = doc(db, SESSIONS, phone);
  const snap = await getDoc(ref);
  if (!snap.exists()) return null;

  // Session shape: { step, data, createdAt, ttl (expiry epoch sec), ... }
  const session = snap.data() as any & { ttl?: number };
  const nowSec = Math.floor(Date.now() / 1000);

  // Purge expired sessions using TTL field
  if (session.ttl && session.ttl < nowSec) {
    await deleteDoc(ref);
    return null;
  }

  return session;
}

// Merge writes so unchanged fields stay intact
export async function saveSession(phone: string, session: any) {
  const ref = doc(db, SESSIONS, phone);
  await setDoc(ref, session, { merge: true });
}

export async function clearSession(phone: string) {
  await deleteDoc(doc(db, SESSIONS, phone));
}

// Initialize a session with Firestore TTL
export async function initializeSession(phone: string, ttlMinutes = 30) {
  const expiresMs = Date.now() + ttlMinutes * 60 * 1000;
  const session = {
    phone,
    step: 'init',
    data: {},
    createdAt: serverTimestamp(),
    expiresAt: serverTimestamp(),   // human‐readable
    ttl: Math.floor(expiresMs / 1000), // Firestore TTL
  };
  await setDoc(doc(db, SESSIONS, phone), session);
  return session;
}

// Finalize signup: create Auth, user doc, business & settings if needed
export async function finalizeSignupAndCleanup(
  phoneNumber: string,
  data: {
    email: string;
    password: string;
    firstName: string;
    lastName: string;
    role: 'business' | 'user';
    companyName?: string;
  }
) {
  try {
    const { email, password, firstName, lastName, role, companyName } = data;
    
    // 1) Create Auth + /users/{uid}
    const user = await signUp({ email, password, firstName, lastName, role });

    // 2) Batch writes
    const batch = writeBatch(db);
    batch.update(doc(db, 'users', user.uid), { telephone: phoneNumber });
    if (role === 'business' && companyName) {
      const businessEmail = email;
      batch.set(doc(db, BUSINESSES, user.uid), {
        companyName,
        businessEmail,
        firstName,
        lastName,
        phoneNumber,
        packages: [],
        photos: ['', '', ''],
        settingsId: `${user.uid}-settings`,
      });
      batch.set(doc(db, SETTINGS, `${user.uid}-settings`), {
        showCompanyDescription: true,
        showCompanyName: true,
        showContact: true,
        showWhoYouAre: true,
        showBackground: true,
      });
    }
    await batch.commit();

    // 3) Notify & cleanup
    const isBusiness = data.role === 'business';
    // const targetUrl = isBusiness ? `${PLATFORM}/company/${user.uid}` : PLATFORM;

    const buuttonLabel = isBusiness
    ? 'Visit Your Business Site'
    : 'Go to Dashboard';

    const promptText = isBusiness 
    ? `Welcome, ${firstName}.\nClick below to view your business site.`
    : `Welcome, ${firstName}.\nTap below to log in and start using the platform.`;

    await sendTextMessage(phoneNumber, `Sign‑up complete! Welcome, ${firstName}.`);
    // await sendUrlButton(phone, targetUrl, buuttonLabel, promptText, `🎉 Sign‑up complete!`, 'Andros');
  
  } catch (err: any) {
    console.error('Signup error:', err);
    await sendTextMessage(phoneNumber, `🚨 There was an issue during sign‑up: ${err.message}`);
  } finally {
    await clearSession(phoneNumber);
  }
}
/**
 * 
 * 
 */